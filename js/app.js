(() => {
    ...
    (() => {
        ...
        class GameRoulette {
            constructor() {
                this.wheelImage = document.querySelector(".wheel-image");
                this.spinButton = document.querySelector(".wheel-spin-button");
                ...
                this.gameCountDisplay.textContent = 2;
                this.spinButton.addEventListener("click", (() => {
                    this.startGame();
                    this.gameCountDisplay.textContent--;
                }));
            ...
            startGame() {
                ...
                if (this.gameIndex >= this.degs.length) {
                    console.log("Игры завершены");
                    return;
                }
                this.isSpinning = true;
                const fullSpins = 10;
                const finalDeg = this.degs[this.gameIndex] + 360 * fullSpins;
                this.wheelImage.style.transition = "transform 5s cubic-bezier(0.33, 0, 0.67, 1)";
                this.wheelImage.style.transform = `rotate(${finalDeg}deg)`;
                setTimeout((() => {
                    this.wheelImage.style.transition = "none";
                    const targetAngle = this.degs[this.gameIndex] % 360;
                    this.wheelImage.style.transform = `rotate(${targetAngle}deg)`;
                    this.isSpinning = false;
                    this.showPopup();
            ...
            showPopup() {
                setTimeout((() => {
                    ...
                    if (this.gameIndex === 0) this.popup1.classList.add("popup_show"); else this.popup2.classList.add("popup_show");
                }), 500);
            }
            nextGame() {
                ...
                if (this.gameIndex < this.degs.length - 1) this.gameIndex++;
                this.isSpinning = false;
            }
        }
        new GameRoulette;
